...
def get_norm_layer(norm_type='instance'):
    ...
    if norm_type == 'batch':
        ...
        def norm_layer(x,track_running_stats=False):
            return Identity()
    else:
...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'linear':
        def lambda_rule(epoch):
            ...
            return lr_l
        scheduler = op.LambdaLR(optimizer, lr_lambda=lambda_rule)
        ...
        scheduler = jt.lr_scheduler.CosineAnnealingLR(optimizer, T_max=opt.n_epochs, eta_min=0)
    else:
