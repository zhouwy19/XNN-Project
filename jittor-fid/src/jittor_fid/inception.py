...
class InceptionV3(Module):
    ...
    def __init__(self, output_blocks=(DEFAULT_BLOCK_INDEX,), resize_input=True, normalize_input=True, requires_grad=False, use_fid_inception=True):
        ...
        assert (self.last_needed_block
                <= 3), 'Last possible output block index is 3'
        ...
        block0 = [inception.Conv2d_1a_3x3, inception.Conv2d_2a_3x3,
                  inception.Conv2d_2b_3x3, nn.Pool(3, stride=2, op='maximum')]
        self.blocks.append(nn.Sequential(*block0))
        if (self.last_needed_block >= 1):
            block1 = [inception.Conv2d_3b_1x1, inception.Conv2d_4a_3x3,
        ...
        if (self.last_needed_block >= 3):
            block3 = [inception.Mixed_7a, inception.Mixed_7b,
    ...
    def execute(self, inp):
        ...
        if self.resize_input:
            x = nn.interpolate(x, size=(299, 299),
...
def _inception_v3(*args, **kwargs):
    """Wraps `jittor.models.inception_v3`

    Skips default weight inititialization if supported by torchvision version.
    See https://github.com/mseitzer/pytorch-fid/issues/28.
    """
    kwargs['init_weights'] = False
    return models.inception_v3(*args, **kwargs)
...
def fid_inception_v3():
    ...
    state_dict = jt.load(FID_WEIGHTS_URL)
    inception.load_parameters(state_dict)
    return inception
...
class FIDInceptionE_1(models.inception.InceptionE):
    ...
    def execute(self, x):
        ...
        branch3x3 = [self.branch3x3_2a(
            branch3x3), self.branch3x3_2b(branch3x3)]
        branch3x3 = jt.concat(branch3x3, dim=1)
        branch3x3dbl = self.branch3x3dbl_1(x)
        branch3x3dbl = self.branch3x3dbl_2(branch3x3dbl)
        branch3x3dbl = [self.branch3x3dbl_3a(
            branch3x3dbl), self.branch3x3dbl_3b(branch3x3dbl)]
