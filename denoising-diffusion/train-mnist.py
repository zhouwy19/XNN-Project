...
channels=1
dim_mults=(1, 2, 2)
train_num_steps=10*6*10**4 # 10 epoch
...
ds = dataset.MNIST(train=True, transform=transform) \
...
trainer = Trainer(
    ...
    ema_decay=0.9999, train_lr=2*10**-4,
    results_folder=f'./results/{dataset_name}/')

# load model with name from folder
#trainer.load(name, folder)
#trainer.load(63, model_ext='pkl')

# this will auto save milestone models and samples
...
sample_save_tile(trainer, num_samples=9*9, save_dir=trainer.results_folder / dataset_name / 'samples/tiles/')

# generate a large set of samples for computing fid, etc.
sample_save_many(trainer, num_samples=50000, save_dir=trainer.results_folder / dataset_name / 'samples')
